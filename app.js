...
function initTimer()
{
  d = getTimeRemaining(deadline);
  if (d.total > 0) 
    {
		console.log(d);
      io.sockets.emit('timeCount', d);
    }
  else 
    {
      io.sockets.emit('timeCount', {'days': 0, 'hours': 0, 'minutes': 0, 'seconds': 0});
      io.sockets.emit('timeOut', 'Time Out');
...
io.sockets.on('connection', function(socket) {
  socket.on('new auction', function(data) {
   ...
   db.test.find().sort({price:-1}).limit(1,function(err,docs){
      //  console.log(docs);
      currentPrice = 0;
      ...
      console.log(data);
          ...
          var record = {time: 0, price: parseInt(data)};
          console.log(record);
